...
from telethon.tl.types import MessageEntityBlockquote, MessageEntityBold, MessageEntityCode, MessageEntityEmail, MessageEntityItalic, MessageEntityMentionName, MessageEntityPre, MessageEntitySpoiler, MessageEntityStrike, MessageEntityTextUrl, MessageEntityUnderline, MessageEntityUrl, TypeMessageEntity, MessageEntityCustomEmoji
from .. import loader

def _add_surrogate(text):
    return ''.join((''.join((chr(y) for y in struct.unpack('<HH', x.encode('utf-16le')))) if 65536 <= ord(x) <= 1114111 else x for x in text))
...
class HTMLParserLib(loader.Library):
    ...
    def unparse(self, text: str, entities: Iterable[TypeMessageEntity], _offset: int=0, _length: Optional[int]=None) -> str:
        """
        ...
        for i, entity in enumerate(entities):
            ...
            while helpers.within_surrogate(text, relative_offset + length, length=_length):
                length += 1
            entity_text = self.unparse(text=text[relative_offset:relative_offset + length], entities=entities[i + 1:], _offset=entity.offset, _length=length)
