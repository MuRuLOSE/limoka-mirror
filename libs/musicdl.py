...
class MusicDLLib(loader.Library):
    ...
    def __init__(self):
        self.config = loader.LibraryConfig(loader.ConfigValue('timeout', 40, 'Timeout for downloading', validator=loader.validators.Integer(minimum=5)), loader.ConfigValue('retries', 3, 'Number of retries for downloading', validator=loader.validators.Integer(minimum=0)), loader.ConfigValue('lossless_priority', False, 'If True, lossless music will be downloaded first', validator=loader.validators.Boolean()))
    ...
    async def dl(self, full_name: str, only_document: bool=False, retries: int=0) -> typing.Union[Document, str]:
        try:
            ...
            if self.config['lossless_priority'] or not document:
                ...
                if not getattr(getattr(result, 'send_message', None), 'reply_markup', None):
                    ...
                    async def handler(event: MessageEdited):
                        nonlocal document
                        try:
                            if event.message.id == m.id and (not getattr(event.message, 'reply_markup', None) or all((button.text != 'Подождите, трек скоро скачается.' for button in utils.array_sum([row.buttons for row in event.message.reply_markup.rows])))) and event.message.document:
        ...
        file.name = 'audio.mp3'
        try:
            skynet = await utils.run_sync(requests.post, 'https://siasky.net/skynet/skyfile', files={'file': file})
