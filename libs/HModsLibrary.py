...
class HModsLib(loader.Library):
    ...
    async def parse_time(self, time_str):
        ...
        if not re.fullmatch('(\\d+[dhms])+', time_str):
            return None
        seconds = 0
        matches = re.findall('(\\d+)([dhms])', time_str)
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, data={'file': open(path, 'rb')}) as response:
                if response.status != 200:
                    os.remove(path)
                    raise aiohttp.ClientResponseError(request_info=response.request_info, history=response.history, status=response.status, message=await response.text(), headers=response.headers)
    ...
    async def get_creation_date(tg_id: int) -> str:
        ...
        headers = {'accept': '*/*', 'content-type': 'application/x-www-form-urlencoded', 'user-agent': 'Nicegram/92 CFNetwork/1390 Darwin/22.0.0', 'x-api-key': 'e758fb28-79be-4d1c-af6b-066633ded128', 'accept-language': 'en-US,en;q=0.9'}
    ...
    async def get_giga_response(self, api_key, query):
        ...
        async with GigaChat(credentials=api_key, scope='GIGACHAT_API_PERS', model=self.config['GIGACHAT_MODEL'], verify_ssl_certs=False) as giga:
    ...
    async def get_giga_models(self, api_key):
        ...
        async with GigaChat(credentials=api_key, scope='GIGACHAT_API_PERS', verify_ssl_certs=False) as giga:
            ...
            if response:
                return [model.id_ for model in response.data] if hasattr(response, 'data') else []
            return None
    ...
    async def virustotal_request(self, session: aiohttp.ClientSession, url: str, headers: Dict[str, str], method: str='GET', data: Optional[Dict[str, Any]]=None, files: Optional[Dict[str, Any]]=None) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            if files:
                ...
                for k, v in files.items():
                    form.add_field(k, v)
                async with session.request(method, url, headers=headers, data=form) as response:
                    logger.debug(f'Response status: {response.status}')
                    logger.debug(f'Response body: {await response.text()}')
                    if response.status == 200:
                        return await response.json()
                    else:
                        logger.error(f'VirusTotal API request failed with status: {response.status}, reason: {response.reason}')
                        return None
            else:
    ...
    async def scan_file_virustotal(self, file_path: str, api_key: str, is_large_file: bool=False) -> Optional[Dict[str, Any]]:
        """
                ...
                with open(file_path, 'rb') as file:
                    ...
                    if is_large_file:
                        ...
                        if not upload_url:
                            ...
                            return None
                        url = upload_url
                    upload_response = await self.virustotal_request(session, url, headers, method='POST', files=files)
                    if upload_response and 'data' in upload_response and ('id' in upload_response['data']):
                        analysis_id = upload_response['data']['id']
                        analysis_url = f'https://www.virustotal.com/api/v3/analyses/{analysis_id}'
                        for attempt in range(20):
                            analysis_response = await self.virustotal_request(session, analysis_url, headers)
                            logger.debug(f'Analysis response (attempt {attempt + 1}): {analysis_response}')
                            if analysis_response and 'data' in analysis_response and ('attributes' in analysis_response['data']) and (analysis_response['data']['attributes'].get('status') == 'completed'):
                                return analysis_response
                            await asyncio.sleep(10)
                        logger.warning(f'Analysis not completed after multiple retries for ID: {analysis_id}')
                        return None
                    else:
                        logger.error(f'File upload or analysis request failed: {upload_response}')
                        return None
    ...
    def format_analysis_results(self, analysis_results: Dict[str, Any]) -> str:
        ...
        """
        if not analysis_results or 'data' not in analysis_results or 'attributes' not in analysis_results['data'] or ('stats' not in analysis_results['data']['attributes']) or ('results' not in analysis_results['data']['attributes']):
            logger.warning(f'Unexpected structure in analysis_results: {analysis_results}')
        ...
        text = f'<b>📊 VirusTotal Scan Results</b>\n\n🦠 <b>Detections:</b> {malicious} / {total_scans}\n🟢 <b>Harmless:</b> {harmless}\n⚠️ <b>Suspicious:</b> {suspicious}\n❓ <b>Undetected:</b> {undetected}\n\n'
        if malicious > 0:
