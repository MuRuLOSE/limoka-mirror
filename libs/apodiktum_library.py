...
from aiogram.utils.exceptions import BotKicked, ChatNotFound, MessageCantBeDeleted, MessageToDeleteNotFound
from telethon.errors import UserNotParticipantError
from telethon.hints import EntityLike
from telethon.tl.functions.channels import CreateChannelRequest, EditAdminRequest, EditBannedRequest, GetFullChannelRequest, InviteToChannelRequest
from telethon.tl.functions.messages import GetDialogFiltersRequest, UpdateDialogFilterRequest
from telethon.tl.types import Channel, Chat, ChatAdminRights, ChatBannedRights, Message, MessageEntityUrl, User
...
class ApodiktumLib(loader.Library):
    ...
    version = __version__
    strings = {'_cfg_cst_auto_migrate': 'Wheather to auto migrate defined changes on startup.', '_cfg_doc_log_channel': 'Wheather to log debug as info in logger channel.', '_cfg_doc_log_debug': 'Wheather to log declared debug messages as info in logger channel.'}
    strings_de = {'_cfg_cst_auto_migrate': 'Ob definierte Änderungen beim Start automatisch migriert werden sollen.', '_cfg_doc_log_channel': 'Ob Debug als Info im Logger-Kanal protokolliert werden soll.', '_cfg_doc_log_debug': 'Ob deklarierte Debug-Meldungen als Info im Logger-Kanal protokolliert werden sollen.'}
    strings_ru = {}
    all_strings = {'strings': strings, 'strings_en': strings, 'strings_de': strings_de, 'strings_ru': strings_ru}

    def __init__(self):
        """
        Initializes the library config.
        """
        self.config = loader.LibraryConfig(loader.ConfigValue('auto_migrate', True, doc=lambda: self.strings('_cfg_cst_auto_migrate'), validator=loader.validators.Boolean()), loader.ConfigValue('log_channel', True, doc=lambda: self.strings('_cfg_doc_log_channel'), validator=loader.validators.Boolean()), loader.ConfigValue('log_debug', False, doc=lambda: self.strings('_cfg_doc_log_debug'), validator=loader.validators.Boolean()))
    ...
    async def __init_classes(self):
        ...
        self.utils_beta = ApodiktumUtilsBeta(self) if await self._internal._beta_access() else None
    ...
    async def __refresh_classes(self):
        ...
        for cl in self.loaded_classes:
            self.utils.log(logging.DEBUG, self.__class__.__name__, f'Refreshing {cl} to the current library state.', debug_msg=True)
            self.loaded_classes[cl] = await self.loaded_classes[cl]._refresh_lib(self)
            if getattr(self, cl):
                setattr(self, cl, self.loaded_classes[cl])
        self.utils.log(logging.DEBUG, self.__class__.__name__, 'Refreshing of all classes done.', debug_msg=True)
    ...
    async def on_lib_update(self, new_lib: loader.Library):
        ...
        new_lib._watcher_q_queue, new_lib._watcher_q_task = self._internal._lib_update_watcher_q_handler()
...
class ApodiktumControllerLoader(loader.Module):
    ...
    def __init__(self, lib: loader.Library):
        ...
        self.utils.log(logging.DEBUG, lib.__class__.__name__, f'class {self.__class__.__name__} is being initiated!', debug_msg=True)
        self.lib = lib
        self._db = lib.db
    ...
    async def _load_github(self):
        ...
        async with aiohttp.ClientSession() as session, session.head(link) as response:
            if response.status >= 300:
                return None
        link_message = await self._client.send_message('me', f'{self.lib.get_prefix()}dlmod {link}')
    ...
    async def _wait_load(self, delay, retries, first_loop=False):
        ...
        while retries:
            ...
            if not getattr(self.lib.lookup('Loader'), 'fully_loaded', False):
                retries = 1
            elif getattr(self.lib.lookup('Loader'), 'fully_loaded', False) and first_loop:
                retries = 0
            else:
                retries -= 1
            if retries != 0:
                self.utils.log(logging.DEBUG, self._libclassname, f'ApoLibController not found, retrying in {delay} seconds...\nHikka fully loaded: {getattr(self.lib.lookup('Loader'), 'fully_loaded', False)}', debug_msg=True)
            if retries == 0:
                return False
...
class ApodiktumWatcherQueue(loader.Module):
    ...
    def register(self, name: str, method: str='q_watcher'):
        ...
        if not hasattr(self.lib.lookup(name), method):
            self.utils.log(logging.ERROR, self.__class__.__name__, f'Module `{name}` has no method called `{method}`!')
            return
        self._watcher_q_queue.setdefault(name, {}).setdefault(method, asyncio.Queue())
        self._watcher_q_task.setdefault(name, {}).setdefault(method, asyncio.create_task(self.__queue_method_handler(name, method)))
    ...
    def unregister(self, name: str, method: str='q_watcher'):
        ...
        self.utils.log(logging.DEBUG, self.__class__.__name__, f'Unregistered method `{method}` for `{name}` from the queue!\nCurrent watcher tasks: {self._watcher_q_task}', debug_msg=True)
        ...
        try:
            while True:
                try:
                    ...
                    await getattr(self.lib.lookup(name), method)(msg) if hasattr(self.lib.lookup(name), method) else self.unregister(name, method)
                except KeyError:
                    self.utils.log(logging.DEBUG, self.__class__.__name__, f'Task stopped! Method `{method}` of `{name}` is not registered!', debug_msg=True)
                    break
                except Exception as exc:
                    self.utils.log(logging.ERROR, name, f'Exception in method `{method}` of `{name}`:\n{exc}', exc_info=True)
                    continue
            return
...
class ApodiktumUtils(loader.Module):
    ...
    def __init__(self, lib: loader.Library):
        ...
        self.log(logging.DEBUG, self._libclassname, f'class {self.__class__.__name__} is being initiated!', debug_msg=True)
    ...
    def log(self, level: Optional[int]=None, name: Optional[str]=None, text: Optional[str]=None, debug_msg: Optional[bool]=False, exc_info: Optional[Exception]=False):
        """
        ...
        if level == logging.INFO:
            return apo_logger.info(text, exc_info=exc_info)
        return apo_logger.debug(text, exc_info=exc_info) if level == logging.DEBUG else None

    async def is_member(self, chat: EntityLike, user: Optional[EntityLike]=None, exp: Optional[int]=5, force: Optional[bool]=False) -> bool:
        """
        ...
        """
        try:
            return await self._client.get_perms_cached(chat, user, exp=exp, force=force)
        except UserNotParticipantError:
            return None

    async def is_linkedchannel(self, chat: EntityLike, user: EntityLike, exp: Optional[int]=300, force: Optional[bool]=False) -> bool:
        """
        ...
        user = await self._client.get_entity(user) if user and isinstance(user, int) else user
        if not isinstance(user, Channel):
            return False
        full_channel = await self._client.get_fullchannel(user, exp=exp, force=force)
        if full_channel and full_channel.full_chat and full_channel.full_chat.linked_chat_id:
            return chat.id == int(full_channel.full_chat.linked_chat_id)

    @staticmethod
    async def get_buttons(message: Message) -> dict:
        """
    ...
    async def get_tag(self, user: EntityLike, WithID: bool=False) -> str:
        """
        ...
        if isinstance(user, Channel):
            if WithID:
                return f"<a href='tg://resolve?domain={user.username}'>{user.title}</a> (<code>{str(user.id)}</code>)" if user.username else f'{user.title}(<code>{str(user.id)}</code>)'
            return f"<a href='tg://resolve?domain={user.username}'>{user.title}</a>" if user.username else f'{user.title}'
        if WithID:
            return f"<a href='tg://resolve?domain={user.username}'>{user.first_name}</a> (<code>{str(user.id)}</code>)" if user.username else f"<a href='tg://user?id={str(user.id)}'>{user.first_name}</a> (<code>{str(user.id)}</code>)"
        return f"<a href='tg://resolve?domain={user.username}'>{user.first_name}</a>" if user.username else f"<a href='tg://user?id={str(user.id)}'>{user.first_name}</a>"
    ...
    async def get_invite_link(self, chat: Union[Chat, int]) -> str:
        """
    ...
    @staticmethod
    def validate_integer(s: Any, digits: Optional[int]=None, minimum: Optional[int]=None, maximum: Optional[int]=None) -> bool:
        """
    ...
    @staticmethod
    def validate_string(s: Any, minimum: Optional[int]=None, maximum: Optional[int]=None, length: Optional[int]=None) -> bool:
        """
    ...
    @staticmethod
    def validate_float(s: Any, minimum: Optional[float]=None, maximum: Optional[float]=None) -> bool:
        """
    ...
    @staticmethod
    def validate_regex(s: Any, regex: str) -> bool:
        """
    ...
    def convert_time(t: Union[str, int]) -> int:
        ...
        try:
            ...
            if 's' in str(t):
                t = int(t[:-1])
            t = int(re.sub('[^0-9]', '', str(t)))
        except ValueError:
            return 0
        return t

    @staticmethod
    def tdstring_to_seconds(tdstr: Any, rev_order: Optional[bool]=False) -> int:
        """
    ...
    def time_formatter(seconds: int, short: Optional[bool]=False) -> str:
        ...
        remainder = seconds
        if short:
            times = {'y': 60 * 60 * 24 * 365, 'w': 60 * 60 * 24 * 7, 'd': 60 * 60 * 24, 'h': 60 * 60, 'm': 60, 's': 1}
        else:
            times = {'millenia': 60 * 60 * 24 * 365 * 1000, 'centuries': 60 * 60 * 24 * 365 * 100, 'decades': 60 * 60 * 24 * 365 * 10, 'years': 60 * 60 * 24 * 365, 'month': 60 * 60 * 24 * 30, 'weeks': 60 * 60 * 24 * 7, 'days': 60 * 60 * 24, 'hours': 60 * 60, 'minutes': 60, 'seconds': 1}
    ...
    def remove_html(self, text: str, escape: Optional[bool]=False, keep_emoji_tag: Optional[bool]=False) -> str:
        """
        ...
        :return: Text
        """
        html_regex = '(<\\/?a.*?>|<\\/?b>|<\\/?i>|<\\/?u>|<\\/?strong>|<\\/?em>|<\\/?code>|<\\/?strike>|<\\/?del>|<\\/?pre.*?>|<\\/?br>)' if keep_emoji_tag else '(<\\/?a.*?>|<\\/?b>|<\\/?i>|<\\/?u>|<\\/?strong>|<\\/?em>|<\\/?code>|<\\/?strike>|<\\/?del>|<\\/?pre.*?>|<\\/?br>|<\\/?emoji.*?>)'
        return (self.escape_html if escape else self.unescape_html)(re.sub(html_regex, '', text))
    ...
    def raw_text(self, message: Message, keep_custom_emoji: Optional[bool]=False) -> str:
        """
    ...
    async def get_first_msg(self, message: Message) -> Message:
        ...
        :return: message
        """
        return await self._client.get_messages(utils.get_chat_id(message), ids=getattr(getattr(message, 'reply_to', None), 'reply_to_top_id', None) or getattr(getattr(message, 'reply_to', None), 'reply_to_msg_id', None))

    async def check_inlinebot(self, chat_id: int, invite_bot: Optional[bool]=True, promote_bot: Optional[bool]=True) -> bool:
        """
    ...
    async def invite_bot(self, chat_id: int) -> bool:
        ...
        """
        try:
            return await self._client(InviteToChannelRequest(chat_id, [self.inline.bot_username]))
        except Exception as exc:
            self.utils.log(logging.DEBUG, self._libclassname, f"Unable to invite inlinebot to {chat_id}. Maybe he's already there?\nError: {exc}", debug_msg=True)
            return False

    async def promote_bot(self, chat_id: int) -> bool:
        """
        ...
        try:
            ...
            return True if bot_perms.is_admin and bot_perms.ban_users and bot_perms.delete_messages else await self._client(EditAdminRequest(channel=chat_id, user_id=self.inline.bot_username, admin_rights=ChatAdminRights(ban_users=True, delete_messages=True), rank='Bot'))
        except Exception as exc:
            self.utils.log(logging.DEBUG, self._libclassname, f'Inlinebot promotion in chat {chat_id} failed!\nError: {exc}', debug_msg=True)
            return False

    async def mute(self, chat_id: int, user_id: int, duration: Optional[int]=0, use_bot: Optional[bool]=True) -> bool:
        """
        ...
        try:
            if use_bot and await self.check_inlinebot(chat_id):
                with contextlib.suppress(Exception):
                    if isinstance(user, Channel):
                        return await self.inline.bot.restrict_chat_member(chat_id if str(chat_id).startswith('-100') else int(f'-100{chat_id}'), user_id if str(user_id).startswith('-100') else int(f'-100{user_id}'), permissions=ChatPermissions(can_send_messages=False), until_date=timedelta(minutes=duration))
                    return await self.inline.bot.restrict_chat_member(chat_id if str(chat_id).startswith('-100') else int(f'-100{chat_id}'), user_id, permissions=ChatPermissions(can_send_messages=False), until_date=timedelta(minutes=duration))
            return await self._client.edit_permissions(chat_id, user_id, timedelta(minutes=duration), send_messages=False)
        except Exception as exc:
            self.utils.log(logging.DEBUG, self._libclassname, f'Unable to mute user {user_id} in chat {chat_id} for duration {duration}min\nError: {exc}', debug_msg=True)
            return False

    async def unmute(self, chat_id: int, user_id: int, use_bot: Optional[bool]=True) -> bool:
        """
        ...
        try:
            if use_bot and await self.check_inlinebot(chat_id):
                with contextlib.suppress(Exception):
                    if isinstance(user, Channel):
                        return await self.inline.bot.restrict_chat_member(chat_id if str(chat_id).startswith('-100') else int(f'-100{chat_id}'), user_id if str(user_id).startswith('-100') else int(f'-100{user_id}'), permissions=ChatPermissions(can_send_messages=True))
                    return await self.inline.bot.restrict_chat_member(chat_id if str(chat_id).startswith('-100') else int(f'-100{chat_id}'), user_id, permissions=ChatPermissions(can_send_messages=True))
            return await self._client.edit_permissions(chat_id, user_id, send_messages=True)
        except Exception as exc:
            self.utils.log(logging.DEBUG, self._libclassname, f'Unable to unmute user {user_id} in chat {chat_id}.\nError: {exc}', debug_msg=True)
            return False

    async def kick(self, chat_id: int, user_id: int, use_bot: Optional[bool]=True) -> bool:
        """
        ...
        try:
            if use_bot and await self.check_inlinebot(chat_id):
                with contextlib.suppress(Exception):
                    if isinstance(user, Channel):
                        return await self.inline.bot.ban_chat_sender_chat(chat_id if str(chat_id).startswith('-100') else int(f'-100{chat_id}'), user_id if str(user_id).startswith('-100') else int(f'-100{user_id}'))
                    return await self.inline.bot.unban_chat_member(chat_id if str(chat_id).startswith('-100') else int(f'-100{chat_id}'), user_id)
            return await self._client.kick_participant(chat_id, user_id)
        except Exception as exc:
            self.utils.log(logging.DEBUG, self._libclassname, f'Unable to kick user {user_id} in chat {chat_id}\nError: {exc}', debug_msg=True)
            return False

    async def ban(self, chat_id: int, user_id: int, duration: Optional[int]=0, use_bot: Optional[bool]=True) -> bool:
        """
        ...
        try:
            if use_bot and await self.check_inlinebot(chat_id):
                with contextlib.suppress(Exception):
                    if isinstance(user, Channel):
                        return await self.inline.bot.ban_chat_sender_chat(chat_id if str(chat_id).startswith('-100') else int(f'-100{chat_id}'), user_id if str(user_id).startswith('-100') else int(f'-100{user_id}'))
                    return await self.inline.bot.kick_chat_member(chat_id if str(chat_id).startswith('-100') else int(f'-100{chat_id}'), user_id, until_date=timedelta(minutes=duration))
            return await self._client(EditBannedRequest(chat_id, user_id, ChatBannedRights(timedelta(minutes=duration), view_messages=True)))
        except Exception as exc:
            self.utils.log(logging.DEBUG, self._libclassname, f'Unable to ban user {user_id} in chat {chat_id} for duration: {duration}min.\nError: {exc}', debug_msg=True)
            return False

    async def unban(self, chat_id: int, user_id: int, use_bot: Optional[bool]=True) -> bool:
        """
        ...
        try:
            if use_bot and await self.check_inlinebot(chat_id):
                with contextlib.suppress(Exception):
                    if isinstance(user, Channel):
                        return await self.inline.bot.unban_chat_member(chat_id if str(chat_id).startswith('-100') else int(f'-100{chat_id}'), user_id if str(user_id).startswith('-100') else int(f'-100{user_id}'))
                    return await self.inline.bot.unban_chat_member(chat_id if str(chat_id).startswith('-100') else int(f'-100{chat_id}'), user_id)
            return await self._client(EditBannedRequest(chat_id, user_id, ChatBannedRights(view_messages=False)))
        except Exception as exc:
            self.utils.log(logging.DEBUG, self._libclassname, f'Unable to unban user {user_id} in chat {chat_id}.\nError: {exc}', debug_msg=True)
            return False

    async def delete_message(self, message: Message, deltimer: Optional[int]=0, use_bot: Optional[bool]=True) -> bool:
        """
        ...
        try:
            ...
            if use_bot and await self.check_inlinebot(chat_id) and isinstance(chat, (Channel, Chat)):
                with contextlib.suppress(MessageCantBeDeleted, BotKicked, ChatNotFound, MessageToDeleteNotFound):
                    return await self.inline.bot.delete_message(chat_id if str(chat_id).startswith('-100') else int(f'-100{chat_id}'), message_id)
            return await self._client.delete_messages(chat_id, message_id)
        except Exception as exc:
            self.utils.log(logging.DEBUG, self._libclassname, f'Unable to delete {message_id} in {chat_id}!\nError: {exc}', debug_msg=True)
            return False

    async def asset_channel(self, title: str, description: str, *, channel: Optional[bool]=False, silent: Optional[bool]=False, archive: Optional[bool]=False, avatar: Optional[str]='', _folder: Optional[str]='') -> Tuple[Channel, bool]:
        """
        ...
        peer = (await self._client(CreateChannelRequest(title, description, megagroup=megagroup))).chats[0]
        if silent:
        ...
        if _folder:
            ...
            try:
                ...
                return
            if any((peer.id == getattr(folder_peer, 'channel_id', None) for folder_peer in folder.include_peers)):
                return
            folder.include_peers += [await self._client.get_input_entity(peer)]
            await self._client(UpdateDialogFilterRequest(folder.id, folder))
        return (peer, True)
...
class ApodiktumUtilsBeta(loader.Module):
    ...
    def __init__(self, lib: loader.Library):
        ...
        self._lib_db = self._db.setdefault(self._libclassname, {})
        self._chats_db = self._lib_db.setdefault('chats', {})
        self.utils.log(logging.DEBUG, self._libclassname, 'Congratulations! You have access to the ApodiktumUtilsBeta!')
...
class ApodiktumInternal(loader.Module):
    ...
    def _lib_update_watcher_q_handler(self):
        self._watcher_q_queue = getattr(self.watcher_q if hasattr(self, 'watcher_q') else None, '_watcher_q_queue', {}) or getattr(self.lib if hasattr(self, 'lib') else None, '_watcher_q_queue', {})
        self._watcher_q_task = getattr(self.watcher_q if hasattr(self, 'watcher_q') else None, '_watcher_q_task', {}) or getattr(self.lib if hasattr(self, 'lib') else None, '_watcher_q_task', {})
        for name in list(self._watcher_q_task):
...
class ApodiktumMigrator(loader.Module):
    ...
    async def migrate(self, classname: str, name: str, changes: dict):
        """
        Migrates a module
    ...
    async def auto_migrate_handler(self, classname: str, name: str, changes: dict, auto_migrate: bool=False):
        """
                ...
                for cnfg_key in self._changes[migration][category]:
                    ...
                    for key, value in configdb.items():
                        ...
                        if value == old_value and key == cnfg_key:
                            ...
                            if self.lib.lookup(new_name) and self.lib.lookup(new_name).config and (key in self.lib.lookup(new_name).config):
